...
def relative_asset(source_path: str, target: str) -> str:
    return "../" * len(Path(source_path).parent.parts) + target
...
def markdown_to_html(source: str, title: str, source_path: str) -> str:
    ...
    for raw in source.splitlines():
        ...
        if heading:
            ...
            slug = re.sub(
                r"[^a-zA-Z0-9\u0600-\u06FF]+", "-", heading_text
            ).strip("-").lower()
            body.append(
                f'<h{level} id="{slug}">{md_inline(heading_text)}</h{level}>'
            )
            continue
        if line.startswith(">"):
            flush_paragraph()
            close_list()
            body.append(
                f"<blockquote>{md_inline(line[1:].strip())}</blockquote>"
            )
            continue
        if re.match(r"^[-*+]\s+", line):
            ...
            body.append(f"<li>{md_inline(item_text)}</li>")
            continue
    ...
    for path in ROOT.rglob("*"):
        ...
        if path.suffix.lower() == ".md":
            ...
            html_target.write_text(
                markdown_to_html(text, title, rel.as_posix()), encoding="utf-8"
            )
